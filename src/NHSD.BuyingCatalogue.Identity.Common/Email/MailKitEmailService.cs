...
namespace NHSD.BuyingCatalogue.Identity.Common.Email
{
    ...
    public sealed class MailKitEmailService : IEmailService
    {
        ...
        [SuppressMessage(
            ...
            Justification =
                "Certificate validation only disabled when specified in configuration (for use in test environments only)")]
        ...
        public async Task SendEmailAsync(EmailMessage emailMessage)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    _settings.Host, _settings.Port, authentication.IsRequired, authentication.UserName, mimeMessage);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "SendEmailAsync: Failed: {server}:{port} with auth required {isRequired} and U:{user}, Sending Message {@mimeMessage}",
                    _settings.Host, _settings.Port, _settings.Authentication.IsRequired, _settings.Authentication.UserName, emailMessage);
                throw;
            }
            finally
            {
