...
@3540
Scenario: User receives email after their account is created
...
@3540
Scenario: User does not receive email when non-authority user tries to create their account
    Given a user is logged in
        | Username            | Password     | Scope        |
...
@3540
Scenario: User does not receive email because of service failure
    Given a user is logged in
        | Username             | Password        | Scope        |
        | PostmanPat@email.com | An0therPa$$w0rd | Organisation |
