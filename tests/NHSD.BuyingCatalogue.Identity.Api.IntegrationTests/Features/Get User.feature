...
@5605
Scenario: Searching for an existing user
...
@5605
Scenario: Searching for a user that does not exist
    Given a user is logged in
        | Username          | Password | Scope        |
        | authority@doe.com | yolo     | Organisation |
...
@5605
Scenario: If a user is not authorised then they cannot search for a user
    Given a user is logged in
        | Username      | Password | Scope        |
...
@5605
Scenario: If a user is not logged in then they cannot search for a user
    When a GET request is made for a user with id 234
