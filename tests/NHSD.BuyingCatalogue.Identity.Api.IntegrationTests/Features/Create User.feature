...
@3538
Scenario: An authority user can create a user
...
@3538
Scenario: Create a new user yields the ID of the new user
    Given a user is logged in
        | Username             | Password        | Scope        |
        | PostmanPat@email.com | An0therPa$$w0rd | Organisation |
    When a POST request is made to create a user for organisation Organisation 2
...
@3538
Scenario: A non authority user cannot create a user
    Given a user is logged in
        | Username            | Password     | Scope        |
...
@3538
Scenario: Create user with valid details when unauthorised
    When a POST request is made to create a user for organisation Organisation 2
        | FirstName | LastName   | PhoneNumber | EmailAddress             |
        | Bob       | Bobkovitch | 0123456789  | bob.bobkovitch@email.com |
