...
@3536
Scenario: An authority user can create a new organisation
    ...
    And the response contains a valid location header for organisation with name Organisation 2
...
@3536
Scenario: An authority user tries to create an existing organisation
    Given a user is logged in
        | Username          | Password       | Scope        |
        | authority@doe.com | Str0nkP4s5w0rd | Organisation |
    When a POST request is made to create an organisation with values
...
@3536
Scenario: A non-authority user can not create a new organisation
    Given a user is logged in
        | Username      | Password     | Scope        |
...
@3536
Scenario: A user that is not logged in can not update organisations
    When a POST request is made to create an organisation with values
        | Name           | OdsCode | PrimaryRoleId | CatalogueAgreementSigned | Line1 | Line2      | Line3        | Line4       | Town  | County         | Postcode | Country |
        | Organisation 2 | Ods 2   | ID 1          | false                    | 12    | Brick Lane | Central Area | City Centre | Leeds | West Yorkshire | LS1 1AW  | England |
